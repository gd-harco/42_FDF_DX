...
static t_nlx_line **get_all_line(t_fdf *fdf, t_vec3d **dmap, int *nb_line, t_img *img);
 ...
 */
t_nlx_line **get_all_line(t_fdf *fdf, t_vec3d **dmap, int *nb_line, t_img *img)
{
	t_nlx_line	**lines;
	int			row;
	int			col;
	...
	*nb_line = 2 * fdf->map->height * fdf->map->width - fdf->map->width - fdf->map->height;
